<template>
  <li role="treeitem"
      ...
      @drop.stop.prevent="handleItemDrop">
    ...
    <div ref="row" :class="anchorClasses" v-on="events">
  ...
  export default {
    ...
    data () {
      return {
        isHover: false,
        isDragEnter: false,
        isDragEnterBefore: false,
        isDragEnterAfter: false,
        model: this.data,
        maxHeight: 0,
        events: {}
      }
    ...
    computed: {
      ...
      classes () {
        return [
          ...
          {'tree-drag-enter-before': this.isDragEnterBefore},
          {'tree-drag-enter-after': this.isDragEnterAfter},
          {'tree-drop-before': true},
    ...
    methods: {
      dragLeaveHandler (e) {
        this.isDragEnterAfter = this.isDragEnterBefore = this.isDragEnter = false
      },
      dragEnterHandler (e) {
        const hasChildren = this.model.getChildren().length > 0
        const row_height = this.$refs.row.clientHeight
        const orderPositionPercent = .15
        console.debug(row_height, e.offsetY, e.offsetX,
          row_height * orderPositionPercent,
          row_height * (1.0 - orderPositionPercent)
        )
        this.isDragEnterAfter = this.isDragEnterBefore = this.isDragEnter = false
        if (e.offsetY <= (row_height * orderPositionPercent)) {
          console.log('drag before', this.model.getText())
          if (hasChildren) {
            this.isDragEnter = true
            return
          }
          this.isDragEnterBefore = true
          return
        }
        if (e.offsetY >= (row_height * (1.0 - orderPositionPercent))) {
          console.log('drag after', this.model.getText())
          this.isDragEnterAfter = true
          return
        }
        this.isDragEnter = true
      },
      ...
      handleItemDrop (e) {
        e.position = null
        e.mode = 'move'
        if (this.isDragEnterAfter) {
          e.position = 'after'
        }
        if (this.isDragEnterBefore) {
          e.position = 'before'
        }
        this.isDragEnter = this.isDragEnterAfter = this.isDragEnterBefore = false
        this.onItemDrop(e, this, this.model)
      }
    },
    created () {
      const self = this
