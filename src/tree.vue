    ...
    methods: {
      findNode (tree, pred) {
        for (let node of tree) {
          ...
          let children = node[this.childrenFieldName]
          if (children && children.length > 0) {
            node = this.findNode(children, pred)
            if (node) {
              return
            }
          }
      ...
      initializeDataItem (item) {
        ...
        let node = Object.assign(new Model(
            ...
            this.async(oriNode, (data) => {
              if (data.length > 0) {
                for (let i in data) {
                  ...
                  var dataItem = self.initializeDataItem(data[i], oriNode)
